...
export default async function EthnicGroupsPage({ searchParams }: Props) {
  ...
  let groups: { id: number; name: string; endonym?: string; clan_count?: number; sub_group_count?: number; sub_group_label?: string; lineage_system_display?: string; countries?: { id: number; name: string }[] }[] = SAMPLE_ETHNIC_GROUPS;
    ...
    <>
      ...
      <div className={styles.page}>
        ...
        <div className={styles.grid}>
          {groups.map((g) => {
            const stats = [];
            if (g.sub_group_count) stats.push({ label: g.sub_group_label || "sub-groups", value: g.sub_group_count });
